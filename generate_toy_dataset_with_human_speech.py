...
def simulate_clip(
    ...
    # Replace the existing audio truncation in simulate_clip():
    sim_audio = room.mic_array.signals  # (C, T_sim)
    T_sim = sim_audio.shape[1]
    if T_sim >= NUM_SAMPLES:
        # Random crop window within the simulated audio
        # Skip the first 50ms (RIR settling) and last 50ms (clean tail)
        settling = int(0.05 * SAMPLE_RATE)
        margin = int(0.05 * SAMPLE_RATE)
        valid_start = settling
        valid_end = T_sim - margin - NUM_SAMPLES
        if valid_end > valid_start:
            start = int(rng.integers(valid_start, valid_end))
        else:
            start = max(0, settling)
